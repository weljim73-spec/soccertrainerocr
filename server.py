...
def extract_match_data(text):
    ...
    # Match overview
    position = extract_value(text, r'\b([A-Z]{1,3})\b.*position', default=None)
    goals = extract_number(text, r'goals[:\s]*(\d+)')
    assists = extract_number(text, r'assists[:\s]*(\d+)')
    athlete_score = extract_number(text, r'athlete.*score[:\s]*(\d+)')
    opposing_score = extract_number(text, r'oppos.*score[:\s]*(\d+)')

    # Skills scores
    two_footed = extract_number(text, r'two[\s-]*footed[:\s]*(\d+)')
    dribbling = extract_number(text, r'dribbling[:\s]*(\d+)')
    first_touch = extract_number(text, r'first\s*touch[:\s]*(\d+)')

    # Additional match metrics would go here...
